...
u8sod u8TestIntegrityFileGz(s8sod *loc_ps8_error)
{
	...
	{
		printf("u8TestIntegrityFileGz: File not present %s ",FICHIER_UPDATE_CPU_BIN);
		loc_u8_resultat = FALSE;
	}
...
u8sod u8TestValidApp(void)
{
	...
	u8sod loc_u8_ordre_tester_crc_data;


	loc_u8_resultat = TRUE; //INIT
	loc_p_handle = NULL; //INIT
	strcpy(loc_ps8_error_gz, "");
	loc_u8_ordre_tester_crc_data = FALSE;	//INIT
	...
	if(NULL != (loc_p_handle = fopen( FICHIER_UPDATE_CPU_BIN, "rb" ))) //CONDITION: fichier présent
	{
		...
		if (sizeof(image_header_t) == fread(&loc_t_header, sizeof(u8sod), sizeof(image_header_t), loc_p_handle))
		{
			...
			else
			{
				...
				{
					printf("OK\n");
					//Vérification CRC data
					loc_u8_ordre_tester_crc_data = TRUE;
				}
			}
		}
		fclose (loc_p_handle);
		
		if(TRUE == loc_u8_ordre_tester_crc_data)
		{
			//On extrait tout du fichier, sauf le header
			printf("Test CRC data de l'application: ...");

			//On copie dans un fichier cpu.bin sans le header => image gz
			CmdSystem_Generique("dd if=%s of=%s skip=1 bs=%d",
								FICHIER_UPDATE_CPU_BIN,
								FICHIER_UPDATE_TMPFILE_GZ,
								sizeof(image_header_t));
			//On vérifie ensuite si le fichier au format gz est correct (on retourne la sortie erreur dans le fichier testgz)
			CmdSystem_Generique("gunzip -t -v %s 2>%s",
								FICHIER_UPDATE_TMPFILE_GZ,
								FICHIER_UPDATE_TESTGZ);

			if(FALSE == u8TestIntegrityFileGz(loc_ps8_error_gz))
			{
				loc_u8_resultat = FALSE;
			}
			//on supprime le fichier
			CmdSystem_Generique("rm %s",FICHIER_UPDATE_TMPFILE_GZ);
				...
				if(u16_cpt_test_update > TIMEOUT_TEST_UPDATE_APP)
				{
					...
					if(loc_u32_size_file > sizeof(image_header_t)) //CONDITION: entete présent
					{
						if(TRUE == u8TestValidApp()) //CONDITION: Fichier valide
						{
							...
							switch(u8_modeboot_encours)//mode où tourne le logiciel en ce momemt
							{
								case MODE_BOOT_AUCUN:
									...
									if(TRUE == u8MiseAJourInfoSystem(MODE_BOOT_APP1, ps8_update__version_be_uboot))
									{
								...
								case MODE_BOOT_APP1:
									//On met à jour APP2
									//On efface la partition dans la FLASH NAND
									CmdSystem_Generique("flash_eraseall -j %s",DEVICE_MTD_NAND_APP2); //JFFS2
									//On copie le fichier en nand
									CmdSystem_Generique("nandwrite -p %s %s",DEVICE_MTD_NAND_APP2,FICHIER_UPDATE_CPU_BIN);
									//Puis on met à jour les info systems
									if(TRUE == u8MiseAJourInfoSystem(MODE_BOOT_APP2, ps8_update__version_be_uboot))
									{
								...
								case MODE_BOOT_APP2:
									//On met à jour APP1
									//On efface la partition dans la FLASH NAND
									CmdSystem_Generique("flash_eraseall -j %s",DEVICE_MTD_NAND_APP1); //JFFS2
									//On copie le fichier en nand
									CmdSystem_Generique("nandwrite -p %s %s",DEVICE_MTD_NAND_APP1,FICHIER_UPDATE_CPU_BIN);
									//Puis on met à jour les info systems
									if(TRUE == u8MiseAJourInfoSystem(MODE_BOOT_APP1, ps8_update__version_be_uboot))
